...
def add_ai_disclosure(input_video: str, output_video: str) -> str:
    ...
    try:
        ...
        # Use metadata approach instead of drawtext to avoid FFmpeg compatibility issues
        ffmpeg_cmd = [
            "ffmpeg", "-y", "-i", input_video,
            "-c:v", "copy",
            "-c:a", "copy",
            "-metadata", "AI-GENERATED=true",
            "-metadata", "COMMENT=This video was generated using AI tools",
            output_video
        ]
        ...
        if result.returncode == 0:
            logger.info(f"✅ AI disclosure metadata added (YouTube 2026 compliant): {output_video}")
            return output_video
        else:
            # If metadata fails, just copy the video unchanged
            logger.warning(f"⚠️ AI disclosure metadata could not be added - ensure manual labeling on YouTube")
            return input_video
...
def _flush_buffer(buffer: list, speaker: str, segments: list):
    ...
    # Multi-Character Voice Parsing (No AI)
    if speaker.lower() == "narrator":
        # Split text by quotes to assign alternating voices
        ...
        for part in parts:
            ...
            if part.startswith('"') or part.startswith('“'):
                clean_dialogue = part.strip('“”"\' ')
                if len(clean_dialogue) > 1:
                    segments.append({
                        "type": "dialogue",
                        "text": clean_dialogue,
                        "speaker": "character",
                        "voice": config.VOICES.get("character", "en-US-GuyNeural"),
                    })
            else:
                segments.append({
                    "type": "narration",
...
def generate_subtitles(
    ...
    MARGIN_V      = 200       # px from bottom — lower third, not edge-hugging
